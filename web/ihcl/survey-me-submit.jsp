...
<%System.out.println("IHCL :::Entering survey-me-submit.jsp for empId : " + empid);
    ...
    System.out.println("IHCL :::Starting survey-me-submit.jsp for empId : " + empid);
    if (!empRating.isEmpty()) {
        try {
            System.out.println("IHCL :::Entering the try block in survey-me-submit.jspfor empId : " + empid);
            ...
            while (keys.hasNext()) {
                System.out.println("IHCL :::survey-me-submit.jsp for empId : " + empid + " iterating through the jsonObj");
                String key = (String) keys.next();
                String rating = jObj.getString(key);
                int questionId = Util.getIntValue(key);
                int ratVal = Util.getIntValue(rating);
                Response respObj = new Response();
                respObj.setQuestionId(questionId);
                respObj.setQuestionType(QuestionType.ME);
                respObj.setResponseValue(ratVal);
                responseList.add(respObj);
                System.out.println("IHCL :::submit-me-submit.jsp ::" + comid + "::" + empid + "::" + questionId + "::" + ratVal);
            }
            ...
            if (subResp) {
                ...
                System.out.println("IHCL :::Successfully saved me response for empId : " + empid);
            } else {
                respJOBJ.put("message", "Saving failed");
                System.out.println("IHCL :::Could not save me response for empId : " + empid);
            }
            out.print(jObj.toString());
            System.out.println("IHCL :::Exiting survey-me-submit.jsp for empId : " + empid);
        } catch (Exception e) {
